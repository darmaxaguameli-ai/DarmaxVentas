                    ...
                    <div className="max-h-96 overflow-y-auto pr-2">
                        {notifications.length > 0 ? (
                            <ul className="space-y-4">
                                {notifications.map(n => (
                                    <li key={n.id} className={`flex items-start gap-3 p-3 rounded-lg transition-colors ${!n.isRead ? 'bg-primary/10 dark:bg-primary/20' : ''}`}>
                                        <span className={`text-xl mt-0.5 ${!n.isRead ? 'animate-pulse' : ''}`}>{getIcon(n.type)}</span>
