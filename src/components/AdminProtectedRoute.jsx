...
const AdminProtectedRoute = ({ children }) => {
  const { isAuthenticated, user, loading, hasPermission, hasModuleAccess } = useAuth();
  ...
  // Usamos hasModuleAccess para permitir el paso si tiene CUALQUIER permiso de gestión
  if (!hasModuleAccess('management')) {
    console.error(`Acceso Denegado a Gestión: El usuario ${user?.name} no cuenta con permisos suficientes.`);
